...
/// <summary>
///     Classe que habilita um host do preview handler em um controle,
/// desde que, o software esteja instalado no computador corrente.
/// </summary>
public class PreviewHandler : Control
{
    ...
    /// <summary>
    /// Gets or sets the background colour of PreviewHandlerHost.
    /// </summary>
    ...
    /// <summary>
    ///     Inicializa uma nova instância para habilitar o host de arquivos do windows.
    /// </summary>
    public PreviewHandler() : base()
    {
    ...
    private Guid GetPreviewHandlerGUID(string filename)
    {
        ...
        if (ext != null)
        {
            ...
            RegistryKey test64B = ext.OpenSubKey("shellex\\{8895b1c6-b41f-4c1c-a562-0d564250836f}");

            if (test64B != null)
                return new Guid(Convert.ToString(test64B.GetValue(null)));

            // sometimes preview handlers are declared on key for the class
            string className = Convert.ToString(ext.GetValue(null));
            if (className != null)
            {
                test64B = Registry.ClassesRoot.OpenSubKey(className + "\\shellex\\{8895b1c6-b41f-4c1c-a562-0d564250836f}");
    ...
    /// <summary>
    /// Resizes the hosted preview handler when PreviewHandlerHost is resized.
    /// </summary>
    /// <param name="e"></param>
    ...
    /// <summary>
    /// Unloads the preview handler hosted in PreviewHandlerHost and closes the file stream.
    /// </summary>
    ...
    internal interface IInitializeWithFile { void Initialize([MarshalAs(UnmanagedType.LPWStr)] string pszFilePath, uint grfMode); }

    [ComImport, InterfaceType(ComInterfaceType.InterfaceIsIUnknown), Guid("b824b49d-22ac-4161-ac8a-9916e8fa3f7f")]
    internal interface IInitializeWithStream { void Initialize(IStream pstream, uint grfMode); }

    [ComImport, InterfaceType(ComInterfaceType.InterfaceIsIUnknown), Guid("7F73BE3F-FB79-493C-A6C7-7EE14E245841")]
    interface IInitializeWithItem { void Initialize(IShellItem psi, uint grfMode); }
