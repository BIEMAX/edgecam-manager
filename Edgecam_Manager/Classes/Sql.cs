 ...
 * 
 * 
 *             Sql - Classe com métodos conexão com banco de dados, seja ele local ou em rede.
 * 
 * 
 ...
 *                  Update      - 02/02/2018 - 09:50 AM - Adicionado tratativas para as exceções no momento de instânciar o objeto Sql- V1.8 Lançada.
 *                  Update      - 04/05/2018 - 03:20 PM - A classe Sql agora herda 'IDisposable' para ser utilizado em elementos 'using' - V1.9 Lançada
 ...
 *                  Update      - 12/02/2019 - 10:49 AM - Corrigido alguns erros de tradução no método 'Sql' - V2.5 Lançada.
 ...
 * 					Update      - 10/06/2019 - 02:31 PM - Melhorado a forma de retorno em caso de exceção no método 'ExecutaSql' - V3.2 Lançada.
 * 					Update      - 14/05/2020 - 09:30 AM - Adicionado o método 'ExecutaMultiSql' - V3.3 Lançada.
 * 
 * 
 ...
 * {
 *          //Instância o objeto
 *          sql = new Sql("String conexao", true);
 *          
 *          //Executa o comando
...
internal class Sql : IDisposable
{
    ...
    public Sql(string Servidor, string DataBase, string User, string Password, Boolean ExecutarTransacao = false)
    {
        try
        {
        ...
        {
            throw new Exception("Erro ao instanciar o objeto Sql", ex);
        }
    }
    ...
    /// <param name="ExecutarTransacao">Caso definido como true, poderá ser utilizado as opções commit e rollback na conexão.</param>
    public Sql(string StrConexao, Boolean ExecutarTransacao = false)
    {
        try
        {
    ...
    public DataTable ExecutaSql(String ComandoSql, Dictionary<String, Object> DicDados = null)
    {
        ...
        {
            throw new Exception("Erro ao executar instrução SQL: " + ex.Message, ex);
        }
        finally
        {
    ...
    /// <summary>
    ///     Método que executa vários comandos SQL em uma única String (ao final de cada comando SQL,
    /// precisa ter obrigatoriamente o ponto e vírgula (;)).
    /// </summary>
    /// <param name="ComandoSql">COmandos SQL</param>
    /// <param name="DicDados">Dicionário de dados.</param>
    /// <returns>DataSet contendo os dados das múltiplas tabelas.</returns>
    public DataSet ExecutaMultiSql(String ComandoSql, Dictionary<String, Object> DicDados = null, Boolean LoadTableNames = false)
    {
        //SqlConnection cn = null;
        ...
        try
        {
            ...
            //String[] cmds = ComandoSql.Split(new char[] { ';' });

            //foreach(String c in cmds)
            //{
            //    if (String.IsNullOrEmpty(c)) continue;
            //    SqlCommand cmd = new SqlCommand(c, _CnSql);
            ...
            if (LoadTableNames)
            {
                String[] tn = ComandoSql.Split(new char[] { ';' });

                for (int x = 0; x < ds.Tables.Count; x++)
                {
                    ds.Tables[x].TableName = tn[x].ToLower().Substring(tn[x].ToLower().IndexOf("from") + 5, tn[x].ToLower().IndexOf("where")).Split(' ')[0];
                }
            }
        }
        catch (Exception ex)
